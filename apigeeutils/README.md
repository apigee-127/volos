...
`npm install -g volos-apigeeutils`

*NOTE*: The `-g` option places the apigeeutils command in your PATH. On "\*nix"-based machines, `sudo` may be required with the `-g` option. If you do not use `-g`, then you need to add the apigeeutils command to your PATH manually. Typically, the `-g` option places modules in: `/usr/local/lib/node_modules/volos-apigeeutils` on *nix-based machines.
...
#### Usage Example
    ...
    var apigeeutils = require('volos-apigeeutils')
